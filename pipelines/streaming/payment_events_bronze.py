...
def _read_payment_events_from_kafka(spark: SparkSession):
    """Create the raw Amazon MSK streaming DataFrame."""

    bootstrap_servers = spark.conf.get("payments.msk.bootstrapServers")

    topic = spark.conf.get("payments.msk.topic")

    service_credential = spark.conf.get("payments.msk.serviceCredential")
...
def payment_events():
    ...
    return parsed_df.select(
        ...
        # Payment event
        # -----------------------------------------------------------
        F.col("parsed.payload.event_id").alias("event_id"),
        F.col("parsed.payload.event_type").alias("event_type"),
        F.to_timestamp(F.col("parsed.payload.event_timestamp")).alias("event_timestamp"),
        F.col("parsed.payload.sequence_number").alias("sequence_number"),
        # -----------------------------------------------------------
        # Transaction
        # -----------------------------------------------------------
        ...
        F.col("parsed.payload.transaction.payment_method").alias("payment_method"),
        F.col("parsed.payload.transaction.status").alias("transaction_status"),
        F.col("parsed.payload.transaction.card_present").alias("card_present"),
        F.col("parsed.payload.transaction.device_id").alias("device_id"),
        F.col("parsed.payload.transaction.ip_address").alias("ip_address"),
        F.col("parsed.payload.transaction.country").alias("country"),
        # -----------------------------------------------------------
        # Synthetic delivery metadata
        # -----------------------------------------------------------
        F.col("parsed.scenario").alias("delivery_scenario"),
        F.to_timestamp(F.col("parsed.simulated_arrival_at")).alias("simulated_arrival_at"),
        # -----------------------------------------------------------
