...
def customers_current():
    ...
    return (
        ...
        .select(
            ...
            F.upper(
                F.trim(F.col("risk_rating"))
            ).alias("risk_rating"),

            F.upper(
                F.trim(F.col("kyc_status"))
            ).alias("kyc_status"),
...
def merchants_current():
    ...
    return (
        ...
        .select(
            ...
            F.upper(
                F.trim(F.col("risk_rating"))
            ).alias("merchant_risk_rating"),

            F.upper(
                F.trim(F.col("status"))
            ).alias("merchant_status"),
...
@dp.materialized_view(
    ...
    comment=(
        "Current fraud-case snapshot used to enrich payment transactions."
    ),
    table_properties={
        "quality": "silver",
        "domain": "fraud",
...
def fraud_cases_current():
    ...
    return fraud_cases.select(
        ...
        F.upper(
            F.trim(F.col("status"))
        ).alias("fraud_case_status"),

        F.trim(
            F.col("suspected_reason")
        ).alias("suspected_reason"),

        F.upper(
            F.trim(F.col("outcome"))
        ).alias("fraud_outcome"),
